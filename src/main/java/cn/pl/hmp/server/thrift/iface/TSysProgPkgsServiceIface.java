...
public class TSysProgPkgsServiceIface implements TSysProgPkgsService.Iface {
    ...
    @Override
    public Map<TPages, List<TSysProgPkgs>> queryByPages(TPages pages)
            throws TException {
        Map<TPages,List<TSysProgPkgs>> tmap = new HashMap<TPages, List<TSysProgPkgs>>();
        TPages tempPage = null;
        Map<Pages,List<SysProgPkgs>> progPkgsMap = progPkgsBusiness.selectByPages(null,ObjectConverter.convet(pages, Pages.class));
