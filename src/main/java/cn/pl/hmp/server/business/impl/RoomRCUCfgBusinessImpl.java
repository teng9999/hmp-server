 ...
 */
@Service
public class RoomRCUCfgBusinessImpl extends AbstractBusiness implements IRoomRCUCfgBusiness {
    @Autowired
    ...
    @Override
    public Map<Pages, List<RoomRCUCfg>> queryPages(RoomRCUCfgExample example, Pages pages) {
    ...
    public int apply2room(String roomType, long hotelId) {
        // 拿到这个房型的模板的所有信息。原始模板
        List<RoomRCUCfg> roomRCUCfgs = queryListByRoomTypeAndHotelId(roomType, hotelId);
        // 拿到该酒店该房型的所有房间。
        HotelRoomExample hotelRoomExample = new HotelRoomExample();
        hotelRoomExample.createCriteria().andHotelIdEqualTo(hotelId).andRoomTypeEqualTo(roomType.trim());
        List<HotelRoom> hotelRooms = hotelRoomMapper.selectByExample(hotelRoomExample);
    ...
    private void removeByHotelAndRoomType(String roomType, long hotelId) {
        ...
        example.createCriteria().andRoomTypeEqualTo(roomType.trim()).andHotelIdEqualTo(hotelId)
     ...
     */
    @Override
    public List<RoomRCUCfg> queryListByRoomTypeAndHotelId(String roomType, long hotelId) {
